...
describe('hechos canónicos del stock para IA', () => {
  ...
  it('proyecta derivedFacts de LOT_HISTORY con diferencia declarado vs verificado', () => {
    const context = buildAiOperationsContext(
      '¿Qué pasó con SHOW-001?', snapshot, '2026-08-24T12:00:00.000Z',
    );
    expect(context.intent).toBe('LOT_HISTORY');
    const kg = context.derivedFacts?.stock.find((fact) => fact.unit === 'kg');
    ...
    expect(kg?.locations).toEqual(expect.arrayContaining([
      expect.objectContaining({
        locationLabel: 'Campo Oriente', declared: 8_000, verified: 7_900, difference: -100, hasDiscrepancy: true,
      }),
      expect.objectContaining({
        locationLabel: 'Frigorífico A', declared: 2_250, verified: 2_250, difference: 0, hasDiscrepancy: false,
      }),
    ]));
  });
