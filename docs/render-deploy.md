...
3. Ingresar `GROQ_API_KEY` cuando Render lo solicite (`sync: false`).
...
- Secretos: rotar `GROQ_API_KEY` desde Environment; no usar variables `VITE_*` para secretos.
...
No hay autenticación ni persistencia de despachos/documentos generados. Antes de exposición productiva, agregar autenticación/autorización y rate limiting a mutaciones y análisis IA.
